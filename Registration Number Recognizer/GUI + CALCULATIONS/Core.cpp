...
void Core::set_svm_model(string file_name)
{
	this->svm.load(file_name.c_str());
	ready = true;
}
void Core::set_image(Mat _image)
{
	this->image = _image;
}

bool Core::plate_detection()
{
...
char Core::character_recognition(Mat img_character)
{
	char c = '*';
...
void Core::do_process()
{
	if (this->plate_detection() && this->plate_recognition())
		done = true;
}

vector<string> Core::get_text_recognition()
{
	if (done)
		return this->text_recognition;
}

vector<Mat> Core::get_plate_detection()
{
	if (done)
		return this->plates;
}

vector<Mat> Core::get_character_isolation()
{
	if (done)
		return this->draw_character;
}

vector<vector<Mat> > Core::get_character()
{
	if (done)
		return this->characters;
}

void Core::sort_character(vector<Mat>& ch)
{
	if (ch.size() < 2) return;
...
vector<double> Core::get_process_time()
{
	return process_time;
}
